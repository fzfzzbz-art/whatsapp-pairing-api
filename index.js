...
<head>
  ...
  <title>واجهة إعدادات الواتساب</title>
  <style>
    :root {
      --bg:#0b0d12;
      --panel:#131722;
...
<body>
  <div class="wrap">
    <section class="hero">
      <h1>واجهة إعدادات الرقم المربوط</h1>
      <p>تم إعادة بناء صفحة الإعدادات بشكل خفيف ومباشر بدون مكتبات خارجية ثقيلة، حتى تعمل بثبات وتتفادى مشكلة الشاشة البيضاء. سجّل الدخول برقمك وكلمة السر ثم عدّل الإعدادات واحفظها.</p>
    </section>
    ...
    <div class="grid">
      <aside class="card">
        ...
        <p class="muted" style="margin-top:0;margin-bottom:16px;line-height:1.8;">اكتب الرقم المربوط وكلمة السر. حقل APP ID اختياري واتركه كما هو إذا كنت تستخدم التطبيق الافتراضي.</p>
        <div class="login-box">
          ...
          <input id="loginApp" class="inp" placeholder="APP ID" value="default" />
          <div class="btn-row">
...
(() => {
  ...
  async function login() {
    ...
    try {
      ...
      renderForm();
      setStatus('تم تسجيل الدخول وتحميل الإعدادات بنجاح.', 'success');
    } catch (error) {
...
function getRobustStatusMessageInfo(msg) {
    ...
    // الدخول في بنية الرسائل العميقة والوسائط المتعددة إذا لم تتوفر المعرفات مباشرة
    if (!info.id || !info.participant) {
        const content = msg?.message ? (msg.message.ephemeralMessage?.message || msg.message) : null;
        if (content) {
            const type = Object.keys(content)[0];
            const mediaMessage = content[type];
            if (mediaMessage?.contextInfo) {
                if (!info.id) info.id = mediaMessage.contextInfo.stanzaId || mediaMessage.contextInfo.id || '';
                if (!info.participant) info.participant = mediaMessage.contextInfo.participant || '';
            }
        }
    }

    // تطهير وتنظيف رقم المشارك ليتوافق مع صيغة سيرفر واتساب الرسمية بالحالات
    if (info.participant) {
        info.participant = String(info.participant).split(':')[0] + '@s.whatsapp.net';
    }
...
async function sendStatusReactionWithFallbacks(sock, phoneNumber, msg, participant = '') {
    try {
        ...
        const finalParticipant = participant || msgInfo.participant;
        const statusMessageId = msgInfo.id;

        if (!finalParticipant || !statusMessageId) return false;

        // اختيار الإيموجي الافتراضي، أو جلب إيموجي الرقم المخصص من الذاكرة إن وجد
        let emoji = '💚';
        try {
            const currentSettings = typeof getActivePhoneSettings === 'function' ? getActivePhoneSettings(phoneNumber) : null;
            if (currentSettings && currentSettings.statusCustomReact) {
                const cleanEmoji = String(currentSettings.statusCustomReact).trim();
                if (cleanEmoji) emoji = cleanEmoji;
            }
        } catch (_) {
            emoji = '💚';
        }

        // إرسال التفاعل بصيغة "reactionMessage" الصارمة والمقبولة فوراً على السيرفر
...
async function handleStatusAction(sock, phoneNumber, msg) {
    try {
        const msgInfo = getRobustStatusMessageInfo(msg);
        const statusMessageId = msgInfo.id;
        const normalizedParticipant = msgInfo.participant;
        const ownJid = sock.user?.id ? sock.user.id.split(':')[0] + '@s.whatsapp.net' : '';

        // التحقق من المعرفات وتجنب التفاعل مع الستوري الخاص بالبوت نفسه منعاً للـ Loop
        if (!statusMessageId || !normalizedParticipant) return;
        if (ownJid && normalizedParticipant.includes(ownJid)) return;

        // نظام منع التكرار السريع (10 ثوانٍ) لضمان دقة الاستجابة ومنع تكرار الإيموجي
        const dedupeKey = `${phoneNumber}_${normalizedParticipant}_${statusMessageId}`;
        if (global.processedStatusEvents && global.processedStatusEvents.has(dedupeKey)) {
            const expiresAt = Number(global.processedStatusEvents.get(dedupeKey) || 0);
            if (expiresAt > Date.now()) return;
            global.processedStatusEvents.delete(dedupeKey);
        }

        // أ) تنفيذ إرسال أمر القراءة والمشاهدة التلقائية أولاً (Read Status)
        const readKey = {
            remoteJid: 'status@broadcast',
            id: statusMessageId,
            participant: normalizedParticipant,
        ...
        try {
            ...
            // معالجة صامتة لضمان استمرار الكود في حال وجود بطء مؤقت بالشبكة
        }

        // ب) تنفيذ إرسال الإيموجي فوراً وبدون أي شروط أو فحص إعدادات معقدة
        const reactedToStatus = await sendStatusReactionWithFallbacks(sock, phoneNumber, msg, normalizedParticipant);

        // ج) تحديث عدادات الإحصائيات العامة للبوت عند نجاح التفاعل
        if (reactedToStatus && typeof incrementAnalytics === 'function') {
            incrementAnalytics('totalStatusReactions');
        }

        // د) حفظ الحدث في الذاكرة المؤقتة لمنع التكرار المزعج لنفس الستوري
        if (global.processedStatusEvents) {
            global.processedStatusEvents.set(dedupeKey, Date.now() + STATUS_EVENT_DEDUPE_TTL_MS);
        }
    } catch (error) {
