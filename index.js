...
function buildPublicLinkedNumberCommands(phone = '') {
    ...
    return [
        '🤖 أوامر الرقم المتاحة:',
        'bot - عرض دليل الأوامر',
        `${prefix}ai on - تشغيل الرد الذكي`,
        `${prefix}ai off - إيقاف الرد الذكي`,
        `${prefix}react on - تشغيل التفاعل بالإيموجي`,
        `${prefix}react off - إيقاف التفاعل بالإيموجي`,
        ...
        '',
        'مثال ترحيب: إذا أرسلت السلام عليكم سيكون الرد: وعليكم السلام ورحمة الله وبركاته 🌷'
    ].join('\n');
}
...
function buildOwnerPairingGuide() {
    return [
        ...
        '.react on - تشغيل التفاعل بالإيموجي على الرسائل',
        '.react off - إيقاف التفاعل بالإيموجي على الرسائل',
...
function buildPhoneSettingsMessage(phone) {
    ...
    return [
        ...
        `😍 التفاعل بالإيموجي: ${settings.autoReact === 'on' ? 'مفعل ✅' : 'متوقف ⛔'}`,
    ...
    return {
        reply_markup: {
            inline_keyboard: [
                ...
                [Markup.button.callback(settings.autoReact === 'on' ? 'إيقاف تفاعل الإيموجي ⛔' : 'تشغيل تفاعل الإيموجي ✅', `emoji_react_toggle_${cleanPhone}`)],
...
async function buildSmartAutoReply(phone, incomingText) {
    const settings = getActivePhoneSettings(phone);
    const fallbackReply = getGuaranteedAutoReply(phone, incomingText);
    ...
    try {
        const prompt = [
            ...
            'إذا كانت الرسالة تحية مثل السلام عليكم فابدأ بوعليكم السلام ورحمة الله وبركاته ثم رحب باختصار.',
            'إذا طلب المستخدم الأوامر أو أرسل bot أو /start فاذكر له: bot و .ai on/off و .react on/off و .statusview on/off و .statusmsg النص.',
...
function buildLinkedNumberCommandsOverview(phone = '') {
    ...
    return [
        ...
        `${prefix}react on - تشغيل التفاعل بالإيموجي على الرسائل`,
        `${prefix}react off - إيقاف التفاعل بالإيموجي على الرسائل`,
        `${prefix}statusview on - تشغيل رسالة مشاهدة الحالة`,
        `${prefix}statusview off - إيقاف رسالة مشاهدة الحالة`,
        `${prefix}statusmsg النص - تغيير رسالة مشاهدة الحالة`
    ].join('\n');
...
function buildTelegramCommandsOverview() {
    return [
        ...
        'ومن الأزرار تحت /start تقدر تدير الذكاء والردود والإعدادات وتفعيل أو إيقاف التفاعل بالإيموجي.'
    ].join('\n');
}
...
function buildAiManagerMessage(phone) {
    ...
    return [
        ...
        'وعند الإيقاف سيستمر الرد الأساسي المختصر على التحيات والأوامر الشائعة فقط.',
        '',
        buildLinkedNumberCommandsOverview(phone)
    ].join('\n');
}
...
function buildEmojiReactManagerMessage(phone) {
    const settings = getActivePhoneSettings(phone);
    return [
        `😍 إدارة التفاعل بالإيموجي للرقم ${phone}`,
        `الحالة الحالية: ${settings.autoReact === 'on' ? 'مفعل ✅' : 'متوقف ⛔'}`,
        `الإيموجي المستخدم حالياً: ${pickRandomStatusEmoji(phone)}`,
        '',
        'عند التفعيل سيتفاعل الرقم تلقائياً بنفس إيموجي الـ API أو من قائمة الإيموجيات المحفوظة لهذا الرقم.',
        '',
        buildLinkedNumberCommandsOverview(phone)
    ].join('\n');
}
...
function getEmojiReactManagerKeyboard(phone) {
    ...
    return {
        reply_markup: {
            inline_keyboard: [
                [
                    Markup.button.callback(settings.autoReact === 'on' ? 'إيقاف التفاعل ⛔' : 'تشغيل التفاعل ✅', `emoji_react_toggle_${cleanPhone}`)
                ],
...
function getStartKeyboard() {
    return Markup.inlineKeyboard([
        ...
        [
            ...
            Markup.button.callback('تفاعل الإيموجي ✨', 'emoji_react_menu')
        ],
...
async function handlePublicLinkedNumberCommand(sock, phoneNumber, msg) {
    ...
    if (/^(?:bot|menu|help|الاوامر|الأوامر|ابدأ|ابدا|start|\/start|\/help)$/i.test(text)) {
        await sock.sendMessage(from, { text: buildPublicLinkedNumberCommands(phoneNumber) }, { quoted: msg });
        return true;
    }

    if (buildOwnerCommandRegex(phoneNumber, 'react', '\\s+(on|off)$').test(text)) {
        const value = text.split(/\s+/)[1].toLowerCase();
        updatePhoneSettings(phoneNumber, { autoReact: value });
        await sock.sendMessage(from, { text: `✅ التفاعل بالإيموجي ${value === 'on' ? 'مفعل' : 'متوقف'} لهذا الرقم.` }, { quoted: msg });
        return true;
    }
...
async function handleIncomingMessage(sock, phoneNumber, msg) {
    try {
        ...
        if (settings.autoReact === 'on' && msg.key) {
...
${buildTelegramCommandsOverview()}`);
    ...
    if (data === 'emoji_react_menu') {
        const phones = getUserPhones(ctx.from.id);
        if (!phones.length) {
            return safeReply(ctx, '❌ لا يوجد لديك رقم مربوط لإدارة التفاعل بالإيموجي.');
        }
        ...
        return safeReply(ctx, '✨ اختر الرقم الذي تريد إدارة التفاعل بالإيموجي له:', { reply_markup: { inline_keyboard: rows } });
    }
    ...
    if (data.startsWith('emoji_react_toggle_')) {
        ...
        const settings = getActivePhoneSettings(phone);
        const nextValue = settings.autoReact === 'on' ? 'off' : 'on';
        updatePhoneSettings(phone, { autoReact: nextValue });
        return safeReply(ctx, buildEmojiReactManagerMessage(phone), getEmojiReactManagerKeyboard(phone));
    }
