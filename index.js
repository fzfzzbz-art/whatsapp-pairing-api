...
function buildAutoReplyMessage(phone) {
    const settings = getActivePhoneSettings(phone);
    const name = settings.name || 'بوت الملك فارس';
    const location = settings.from || 'اليمن';
    const age = settings.age || '24';
    const prefix = settings.prefix || '.';
    const footer = settings.footer2 || 'بوت الملك فارس';
    const botLink = getTelegramBotLink();
    return [
        `مرحباً! أنا ${name}.`,
        '🧠 تم تفعيل الرد الذكي التلقائي لهذا الرقم.',
        `📍 الموقع: ${location}`,
        `🎂 العمر: ${age}`,
        `⚙️ البادئة: ${prefix}`,
...
function getGuaranteedAutoReply(phone) {
    return buildConfiguredAutoReplyMessage(phone) || buildAutoReplyMessage(phone);
}
        ...
        const req = https.request(url, {
            ...
            res.on('end', () => {
                if (res.statusCode < 200 || res.statusCode >= 300) {
                    return reject(new Error(`Gemini HTTP ${res.statusCode || 500}`));
                }
                try {
...
function buildPairingApiDescriptor(phone = '') {
    ...
    const prefix = settings.prefix || '.';
    return {
        endpoint: `${PUBLIC_BASE_URL}${PAIRING_API_ROUTE}`,
        ...
        linkedNumberCommands: [
            'bot',
            CHANNEL_LIKE_COMMAND,
            `${prefix}ai on`,
            `${prefix}ai off`,
            `${prefix}statusview on`,
            `${prefix}statusview off`,
            `${prefix}statusmsg النص`
        ],
...
async function handleOwnerControlMessage(sock, phoneNumber, msg) {
    ...
    if (buildOwnerCommandRegex(phoneNumber, '(?:fares|gna)').test(text)) {
        clearOwnerReactionFlow(phoneNumber);
    ...
    if (buildOwnerCommandRegex(phoneNumber, 'statusview', '\\s+(on|off)$').test(text)) {
    ...
    if (buildOwnerCommandRegex(phoneNumber, 'statusmsg', '\\s+').test(text)) {
        const messageText = text.replace(buildOwnerCommandRegex(phoneNumber, 'statusmsg', '\\s*'), '').trim();
        if (!messageText) {
    ...
    if (buildOwnerCommandRegex(phoneNumber, 'ai', '\\s+(on|off)$').test(text)) {
        const value = text.split(/\s+/)[1].toLowerCase();
...
function buildPhoneSettingsMessage(phone) {
    ...
    return [
        ...
        `📜 أوامر الرقم: ${pairingApi.linkedNumberCommands.join(' | ')}`,
        'هذه الكلمة خاصة بهذا الرقم فقط.',
        '',
...
async function buildSmartAutoReply(phone, incomingText) {
    const settings = getActivePhoneSettings(phone);
    const fallbackReply = getGuaranteedAutoReply(phone);
    if (settings.aiReply !== 'on' || !GEMINI_API_KEY) {
        return fallbackReply;
    }
    try {
        const prompt = [
            `أنت مساعد واتساب عربي باسم ${settings.name || 'بوت الملك فارس'}.`,
            ...
            'إذا سأل المستخدم عن الأوامر فاذكر له: bot و .ai on/off و .statusview on/off و .statusmsg النص و .fares.',
            'رسالة المستخدم:',
            String(incomingText || '').slice(0, 1200)
        ].join('\n');
        const data = await callGeminiGenerate(prompt);
