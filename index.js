...
function buildPublicLinkedNumberCommands(phone = '') {
    const botLink = getTelegramBotLink();
    return [
        '🤖 هذا الرقم يعمل تلقائياً.',
        '⚙️ جميع إعدادات الرقم المربوط تُدار من بوت تيليجرام فقط.',
        botLink ? `🔗 رابط البوت: ${botLink}` : '',
        `📢 رابط قناة واتساب: ${WHATSAPP_CHANNEL_LINK}`
    ].filter(Boolean).join('\n');
}
...
function buildLinkedNumberWelcomeMessage() {
    ...
    return [
        '✅ تم تفعيل رقمك بنجاح',
        botLink ? `🔗 ربط البوت: ${botLink}` : '',
        `📢 رابط قناة الواتساب: ${WHATSAPP_CHANNEL_LINK}`
    ].filter(Boolean).join('\n');
}
    ...
    return {
        reply_markup: {
            inline_keyboard: [
                ...
                [Markup.button.url('Contact Save 📇', 'https://bot.goldenqueen.store/contactsave')]
            ]
        }
    };
}
...
async function buildSmartAutoReply(phone, incomingText) {
    return getGuaranteedAutoReply(phone, incomingText);
}
...
function buildLinkedNumberCommandsOverview(phone = '') {
    return [
        '📲 أوامر الرقم المربوط:',
        '❌ تم حذف جميع أوامر التحكم من الرقم المربوط.',
        '✅ التحكم الكامل صار من بوت تيليجرام ولوحة الإعدادات فقط.'
    ].join('\n');
}
...
app.get('/settings', (req, res) => {
    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    res.send(buildUnifiedSettingsHubHTML());
});
...
const PythonMergedLayer = (() => {
    ...
    const TARGET_SITE_BASE_URL = "https://bot.goldenqueen.store";
