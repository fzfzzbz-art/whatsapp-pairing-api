...
// [MEM-OPT] Enable pruning by default and tighten limits for 100-number scale
...
// [MEM-OPT] Pruning enabled by default - essential for 100-number scale
const SESSION_PRUNE_ENABLED = ['1', 'true', 'yes', 'on'].includes(String(process.env.SESSION_PRUNE_ENABLED || 'true').trim().toLowerCase());

// [MEM-OPT] Tighter per-session storage cap: 4MB default (was 12MB)
const SESSION_STORE_MAX_BYTES = Math.max(256 * 1024, Number(process.env.SESSION_STORE_MAX_BYTES || (4 * 1024 * 1024)));

// [MEM-OPT] Tighter key limits to keep sessions lean
const SESSION_MAX_PRE_KEYS = Math.max(20, Number(process.env.SESSION_STORE_MAX_PRE_KEYS || 200));       // was 10000
const SESSION_MAX_SIGNAL_SESSIONS = Math.max(20, Number(process.env.SESSION_STORE_MAX_SIGNAL_SESSIONS || 200)); // was 10000
const SESSION_MAX_SENDER_KEYS = Math.max(20, Number(process.env.SESSION_STORE_MAX_SENDER_KEYS || 200));  // was 10000

// [MEM-OPT] Reuse single MongoDB connection - never create more than 1 connection per process
...
function pruneSessionFiles(files = {}) {
  ...
  // [MEM-OPT] Always prune regardless of SESSION_PRUNE_ENABLED for memory safety
  const keep = {};
...
async function getSessionCollection() {
  ...
  if (!collectionPromise) {
    collectionPromise = (async () => {
      // [MEM-OPT] Reuse single client instance - avoid creating new connections per call
      if (!_mongoClient) {
        _mongoClient = new MongoClient(MONGODB_URI, {
          ...
          // [MEM-OPT] Minimal pool - 1 connection is enough for sequential ops
          maxPoolSize: 3,
          minPoolSize: 1,
          retryWrites: true,
          // [MEM-OPT] Auto-compress wire protocol
          compressors: ['zlib'],
        });
        await _mongoClient.connect();
      }

      const db = _mongoClient.db(MONGODB_DB_NAME);
