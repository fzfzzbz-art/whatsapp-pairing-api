...
TARGET_SITE_BASE_URL = (os.getenv("TARGET_SITE_BASE_URL") or PUBLIC_BASE_URL or INTERNAL_PAIRING_BASE_URL).strip().rstrip("/") or INTERNAL_PAIRING_BASE_URL
TARGET_PAIRING_API_URL = (os.getenv("TARGET_PAIRING_API_URL") or f"{INTERNAL_PAIRING_BASE_URL}/api/pairing").strip() or f"{INTERNAL_PAIRING_BASE_URL}/api/pairing"
...
LEGACY_PAIR_API_URLS = {
    "https://bwt-lwts.onrender.com/api/pairing",
...
def resolve_pair_code_api_url() -> str:
    ...
    if GREEN_API_ID_INSTANCE and GREEN_API_TOKEN_INSTANCE:
        auto_green_url = get_green_api_authorization_url().rstrip("/")
        if configured_url in {"", TARGET_PAIRING_API_URL.rstrip("/"), auto_green_url}:
            return auto_green_url
    if configured_url and configured_url not in {url.rstrip("/") for url in LEGACY_PAIR_API_URLS}:
...
async function readSessionIndex() {
  ...
  try {
    return await fs.readJson(SESSION_INDEX_FILE);
  } catch (_) {
    return { sessions: {} };
  }
}
...
async function closeSocket(phone) {
  const normalized = normalizePhone(phone);
...
async function createSocket(phone) {
  ...
  const startPromise = (async () => {
    ...
    sock.ev.on('creds.update', async () => {
      try {
        await saveCreds();
        await updateSessionIndex(normalized, { registered: !!sock.authState?.creds?.registered, lastCredsSaveAt: new Date().toISOString() });
      } catch (error) {
    ...
    await updateSessionIndex(normalized, { registered: !!sock.authState?.creds?.registered, sessionDir });
    return sock;
  })().finally(() => {
...
app.all('/api/pairing', async (req, res) => {
  ...
  try {
    ...
    if (sock.authState?.creds?.registered) {
      await updateSessionIndex(phone, { registered: true, alreadyLinked: true, connected: Number(sock.ws?.readyState) === 1 });
    ...
    await updateSessionIndex(phone, { registered: false, lastPairCodeRequestedAt: new Date().toISOString() });
    return res.json({ success: true, linked: false, number: phone, code: String(code || '').trim() });
  } catch (error) {
    await closeSocket(phone);
    await updateSessionIndex(phone, { registered: false, connected: false, lastError: error?.message || String(error), lastErrorAt: new Date().toISOString() });
...
process.on('SIGINT', async () => {
  for (const phone of Array.from(sockets.keys())) await closeSocket(phone);
  process.exit(0);
});
...
def ensure_embedded_companion_files(base_dir: Optional[Path] = None) -> dict[str, Path]:
    ...
    protected_existing_files = {"package.json", "server.js", "index.html"}
    written_files: dict[str, Path] = {}
    for filename, content in embedded_files.items():
        file_path = target_dir / filename
        if file_path.exists() and filename in protected_existing_files:
...
def stop_embedded_companion_process() -> None:
    global EMBEDDED_COMPANION_PROCESS
    process = EMBEDDED_COMPANION_PROCESS
...
def start_embedded_companion_process() -> None:
    global EMBEDDED_COMPANION_PROCESS
    ...
    env = os.environ.copy()
    env.setdefault("PAIRING_SERVER_PORT", str(COMPANION_PORT))
    env.setdefault("COMPANION_PORT", str(COMPANION_PORT))
...
def main():
    ...
    ensure_embedded_companion_files()
    start_embedded_companion_process()
