...
DEFAULT_REMOTE_PAIRING_BASE_URL = "https://bwt-lwts.onrender.com"
...
def sync_selected_number_status_react_emojis(user_id: int, target_number: str, emojis: list[str]) -> None:
    cleaned_emojis = split_status_custom_react_emojis(" ".join(str(item or "").strip() for item in (emojis or [])))
    if not cleaned_emojis:
        return
    ...
    if not normalized_number or not record_belongs_to_user(record, user_id):
        sync_user_status_react_emojis_to_site(user_id, cleaned_emojis)
        return

    payload = dict(record) if isinstance(record, dict) else {}
    payload["emoji"] = cleaned_emojis[0]
    payload["updated_at"] = datetime.now(timezone.utc).isoformat()
    LINKED_WHATSAPP_USERS[normalized_number] = payload
    save_linked_whatsapp_users()

    pending_payload = PENDING_PAIRINGS.get(normalized_number)
    if isinstance(pending_payload, dict):
        pending_payload["emoji"] = cleaned_emojis[0]
...
def delete_pairing_session_sync(number: str) -> dict[str, Any]:
    ...
    for base in (get_url_base(resolve_pair_code_api_url(), TARGET_SITE_BASE_URL), INTERNAL_PAIRING_BASE_URL, TARGET_SITE_BASE_URL):
...
def resolve_pair_code_api_url() -> str:
    configured_url = str(SETTINGS.get("pair_code_api_url") or "").strip().rstrip("/")
    normalized_internal_url = f"{INTERNAL_PAIRING_BASE_URL}/api/pairing".rstrip("/")
    legacy_urls = {url.rstrip("/") for url in LEGACY_PAIR_API_URLS}
    if configured_url and configured_url not in legacy_urls and configured_url != normalized_internal_url:
...
def fetch_pairing_status_sync(number: str) -> dict[str, Any]:
    ...
    api_base_url = get_url_base(resolve_pair_code_api_url(), TARGET_SITE_BASE_URL) or TARGET_SITE_BASE_URL
    candidate_urls: list[str] = []
    for candidate_base in (api_base_url, TARGET_SITE_BASE_URL):
...
async def user_emoji_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message and update.effective_user:
        await show_owned_numbers_panel(update.message, update.effective_user.id, purpose="emoji")
...
async def handle_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if (query.data in {"pair_code", "refresh_home", "open_drf", "user_set_emoji", "user_status_custom_react", "my_linked_numbers", "unlink_my_number"} or query.data.startswith("pair_lang:") or query.data.startswith("owned_") or query.data.startswith("unlink_number:") or query.data.startswith("emoji_number:")) and not is_admin(update):
        if not await ensure_subscription(update, context):
            return
    ...
    if query.data == "user_set_emoji":
        ...
        user = update.effective_user
        if user:
            await show_owned_numbers_panel(query.message, user.id, purpose="emoji")
        return
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("awaiting_user_emoji"):
        ...
        save_user_emoji_settings()
        if selected_number:
            sync_selected_number_status_react_emojis(user.id, selected_number, emoji_list[:10])
        else:
            update_linked_user_emoji(user.id, primary_emoji)
            sync_user_status_react_emojis_to_site(user.id, emoji_list[:10])
        context.user_data.pop("awaiting_user_emoji", None)
        context.user_data.pop("selected_emoji_number", None)
        success_tail = f"\n📞 الرقم: {selected_number}" if selected_number else ""
        await update.message.reply_text(
            f"✅ تم حفظ رموز الحالة بنجاح: {', '.join(emoji_list[:10])}\n"
            f"العدد الحالي: {len(emoji_list[:10])}/10"
            f"{success_tail}",
...
def main():
    ...
    # If hosting is a Python-only platform (no Node.js) or the operator
    # explicitly disables it, skip the embedded companion and fall back to the
    # external pairing API so the bot still works.
    disable_embedded = (
        os.getenv("DISABLE_EMBEDDED_COMPANION", "").strip().lower() in ("1", "true", "yes", "on")
        or os.getenv("PYTHON_ONLY_HOSTING", "").strip().lower() in ("1", "true", "yes", "on")
        or shutil.which("node") is None
    )

    resolved_pair_api_url = resolve_pair_code_api_url().rstrip("/")
    internal_pair_api_url = f"{INTERNAL_PAIRING_BASE_URL}/api/pairing".rstrip("/")
    embedded_companion_started = True
    if disable_embedded and resolved_pair_api_url == internal_pair_api_url:
        logger.info("Embedded companion is disabled for this host; using external pairing API instead.")
        SETTINGS["pair_code_api_url"] = TARGET_PAIRING_API_URL
        resolved_pair_api_url = TARGET_PAIRING_API_URL.rstrip("/")
        PAIRING_RUNTIME_DISABLED_REASON = ""
        save_settings(SETTINGS)
    elif resolved_pair_api_url == internal_pair_api_url:
        ensure_embedded_companion_files()
        embedded_companion_started = start_embedded_companion_process()
        if not embedded_companion_started:
            logger.warning("Embedded companion is unavailable: %s", PAIRING_RUNTIME_DISABLED_REASON or "unknown error")
            # Don't crash the bot — fall back to external pairing API
            SETTINGS["pair_code_api_url"] = TARGET_PAIRING_API_URL
            save_settings(SETTINGS)
            resolved_pair_api_url = TARGET_PAIRING_API_URL.rstrip("/")
            PAIRING_RUNTIME_DISABLED_REASON = ""
        ...
        PAIRING_RUNTIME_DISABLED_REASON = ""
        logger.info("Using external pairing API: %s", resolved_pair_api_url)
